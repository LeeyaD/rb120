...
# #send is an instance method that is defined in the module Mailing which we've only mixed into one subclass of CommunicationsProvider. And so since #send doesn't exist in the PostalService class, Ruby is following the method lookup path and is actually invoking Object#send which is reading the first arg 860 Blackbird Ln.' as a method. This explains why we're receiving the undefined method argument.
# To fix this we should mix module Mailing into PostalService class too
# SOLUTION also states to avoid accidental method overide, don't name them the same as methods further up in the lookup path
class Mail
  def to_s
    "#{self.class}"
  end
